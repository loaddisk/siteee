...
```html
{{echo}}{{This is its own thing, not javascript}}{{/echo}}
```
### Detailed design
...
---
 Test cases should be added for both compiler and language server to ensure invalid html `<%%>` elements and double braces `{{ echo }}{{}}{{/echo}}` are still rendered correctly, if in CustomElements.
### Drawbacks
